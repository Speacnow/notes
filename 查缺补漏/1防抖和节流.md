...
个人理解：函数防抖就是，一段时间一个连续的操作，多次触发事件绑定的回调函数，变为只触发一次。解决方法：将回调函数使用setTimeOut延迟执行，如果又触发了回调函数，则cleartimer(timer),然后再将timer赋值为setTimeout(回调函数)
...
function debounce(fn, delay) {
    ...
    return function () {
        var _this = this; // 取debounce执行作用域的this,谁调用了debounce函数，this就指向了谁，说明定义时候是不能确定this
        var args = arguments;
        if (timer) {
            clearTimeout(timer);
        }
...
> 每隔一段时间，只执行一次函数。主要是不用cleartimer
...
文档中出现的源代码都在这里: [防抖](https://github.com/dzfrontend/blog/blob/optimization/Optimization/函数防抖和节流/debounce.js)、[节流](https://github.com/dzfrontend/blog/blob/optimization/Optimization/函数防抖和节流/throttle.js)
...
[防抖、节流](http://alloween.top/2018/04/16/防抖、节流/)

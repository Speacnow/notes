...
创建渲染树后，下一步就是布局（Layout）,或者叫回流（reflow,relayout），这个过程就是通过渲染树中渲染对象的信息，计算出每一个渲染**对象的位置和尺寸**，将其安置在浏览器窗口的正确位置，而有些时候我们会在文档布局完成后**对DOM进行修改，这时候可能需要重新进行布局，也可称其为回流**，本质上还是一个布局的过程，**每一个渲染对象都有一个布局或者回流方法**，实现其布局或回流。

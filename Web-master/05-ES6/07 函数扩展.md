...
另外，一个容易忽略的地方是，参数默认值不是传值的，而**是每次都重新计算默认值表达式的值**。也就是说，参数默认值是惰性求值的。
...
（1）函数体内的`this`对象，就是定义时作用域所指向的对象，而不是使用时所在的对象。

 箭头函数没有this，箭头函数执行时不向箭头参数传this，故想确定箭头函数内的this，要看外层的this

 确定方法一：确定代码执行时，最近外层函数（不考虑箭头函数做形参的情况）的this，箭头函数里this继承这个this 

 确定方法二：函数体内的`this`对象，**就是定义生效时，所在的作用域指向的（this）对象**

 确定方法三：箭头函数`this`总是指向**函数定义生效时所在的对象**（不是函数对象）

最终确定方法： 箭头函数没有this，只能继承别人的this,所以在当前作用域下找this（一般属于是所在函数，因为每个函数都有this arguments等属性）。函数this的值需要在代码执行中确定。
...
//只有普通函数用bind apply call有意义，因为他们有this。但是箭头函数bind apply call只是改了个寂寞
```

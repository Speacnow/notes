...
个人补充：then()中返回的值，会通过Promise.resolve()包装生成新的promise.（而且Promise.resolve()有幂等性），所以后面的then就是这个新promise的回调函数。
...
`Promise.all()`方法用于将**多个 Promise 实例**，包装成一个新的 Promise 实例。
...
上面代码中，只要`p1`、`p2`、`p3`之中有一个实例率**先改变状态**，`p`的状态就跟着改变。那个率先改变的 Promise 实例的返回值，就传递给`p`的回调函数。

...
个人理解：Generator 就好像是一个遍历器生成函数，调用它会产生一个遍历器。然后yield就是每个next（）执行暂停的位置。
...
个人理解：yield只会向外（在外调用next）返回{value;done},对生成器内部总是不返回值，（比如 a= yiled 表达式，a为空），怎么让a 有值呢，就是yield向内返回值呢？答案就是next传参

`yield`表达式本身没有返回值，或者说总是返回`undefined`。`next`方法可以带一个参数，**该参数就会被当作上一个`yield`表达式的返回值。** 然后会执行上一个yield与本次yeild之间的代码，再计算出本次yeild返回什么值

个人理解：next参数是已经执行完毕的yield的返回值，因为遍历器调用next时，开始时指针指向上一次执行完毕停留的位置，也就是上一个yield的位置，然后吧next参数传进来，成为上一个yield的返回值。然后代码开始从当前指针所在的表达式开始执行，停留在下一个yield
...
function request(url) {
    ...
    //如果直接it.next()的话,就会报错，必须要向上面一样异步
    //关于为什么异步 我猜 ：执行进入异步之前，本次的同步函数执行完了，代表本次的yield执行完毕。然后异步里面的it.next开了一个单独的线程执行it.next(),就像在外部又执行了it.next()一样
    //it.next('response');
}

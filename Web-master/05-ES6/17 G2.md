...
个人理解 就是一个函数可以暂停执行，跳出去执行下一步代码，然后等到合适的时机，将暂停函数重新开始执行
...
（个人理解，thunk（）返回值才是thunk函数，thunk函数特点就是 单参数 和 参数是回调函数）
...
任何函数，只要参数有回调函数，就能写成 Thunk 函数的形式。下面是一个简单的 **Thunk 函数转换器**，生成器，不是Thunk函数本身。
...
//readFileThunk(fileA)这个才是Thunk函数
```
...
var r1 = g.next();
//r1.value是一个thunk函数，所以一个函数的回调行为可以封装到iterator的返回值value中，在Generator函数外侧提取出来，实现如异步等耗时的操作
r1.value(function (err, data) {
  if (err) throw err;
  var r2 = g.next(data);//将读取的数据传回Generator函数中

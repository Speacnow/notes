...
解析器在**调用函数**每次都会向函数内部**传递**进一个隐含的参数，这个隐含的参数就是 this(传递的this来自外部 比如obj.fu()传递的是obj)，this 指向的是一个对象，**这个对象我们称为函数执行的上下文对象**。

**this永远指向执行时挨的最近的对象,谁导致函数执行，this就指向谁**

本人添加：https://www.cnblogs.com/pssp/p/5216085.html
...
ES6 中的箭头函数并不会使用上面的准则，而是会继承外层函数调用的 this 绑定（无论 this 绑定到什么）。因为箭头函数没有this，所以会演着作用域去找this,也就是父级的this
